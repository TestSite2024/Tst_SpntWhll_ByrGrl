...
window.onload = async () => {
  ...
  function process(props) {
    ...
    for(var i=0;i<props.items.length;i++){
        ...
        props.items[i].label = window.atob( props.items[i].label);
    }
  }
  ...
  async function loadImages(images, props) {
  const promises=[];

    if (images instanceof HTMLImageElement) promises.push(images.decode());
    promises.push(process(props));
  try {
...
var Wheel = class {
    ...
    resize() {
        ...
        for (let a of this._items) this._itemLabelFontSize = Math.min(this._itemLabelFontSize, E(a.label, this.itemLabelFont, this._labelMaxWidth, this._context));
        this.refresh()
    }
    draw(e = 0) {
    ...
    drawItemBackgrounds(e, t = []) {
        ...
        for (let [n, r] of t.entries()) {
            let a = this._items[n];
            e.fillStyle = (s = a.backgroundColor) != null ? s : this._itemBackgroundColors[n % this._itemBackgroundColors.length], e.fill(a.path)
        }
    }
    drawItemImages(e, t = []) {
        //console.log(t);
    ...
    drawItemLabels(e, t = []) {
        ...
        for (let [a, d] of t.entries()) {
            ...
            if (this._itemLabelStrokeWidth > 0 && (e.lineWidth = r, e.strokeStyle = this._itemLabelStrokeColor, e.lineJoin = "round", e.strokeText(h.label, 0, s)), e.fillStyle = C, e.fillText(h.label, 0, s), this.debug) {
